# python-signal-processing-basics
Generazione di un segnale sinusoidale, aggiunta di rumore gaussiano e filtraggio tramite media mobile.

## Descrizione
Questo progetto genera un segnale sinusoidale, aggiunge rumore gaussiano e applica una media mobile per filtrarlo. L’obiettivo è mostrare in modo semplice come creare, contaminare e ripulire un segnale usando le librerie NumPy e Matplotlib.

### Funzionalità
1. Generazione di un asse temporale personalizzabile
2. Creazione di un segnale sinusoidale
3. Aggiunta di rumore gaussiano
4. Applicazione di un filtro a media mobile
5. Visualizzazione dei risultati

## Requisiti
- Python 3.x
- NumPy
- Matplotlib

## Installazione:
```
pip install numpy matplotlib
```
# Esecuzione
Lancia il file:
```
python segnale_rumoroso_media_mobile.py
```

Il programma chiederà:
- numero di campioni
- ampiezza
- frequenza
- deviazione standard del rumore

## Output
Il programma mostra un grafico con:
- segnale rumoroso
- segnale filtrato tramite media mobile
